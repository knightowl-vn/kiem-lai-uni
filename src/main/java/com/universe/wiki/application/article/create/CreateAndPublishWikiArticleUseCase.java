...
@Service
public class CreateAndPublishWikiArticleUseCase {
    ...
    public WikiArticleDTO execute(
            CreateAndPublishWikiArticleCommand command
    ) {
        Objects.requireNonNull(
                command,
                "Create and publish wiki article command "
                        + "không được để trống."
        );
        ...
        /*
         * Không createDraft() rồi publish().
         *
         * Article sinh ra trực tiếp ở PUBLISHED
         * với aggregateVersion = 1.
         */
        WikiArticle article =
    ...
    private void saveInitialRevision(
        ...
        WikiArticleRevision revision =
                WikiArticleRevision.createSnapshot(
                        revisionId,
                        article,
                        RevisionChangeType.CREATE_AND_PUBLISH,
                        editSummary
                );
