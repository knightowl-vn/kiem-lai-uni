...
public class PublicWikiController {
    ...
    private final ListPublishedWikiArticlesUseCase
            listPublishedArticlesUseCase;

    private final GetPublishedWikiArticleUseCase
            getPublishedArticleUseCase;

    private final ArticleTypePathMapper
            articleTypePathMapper;

    private final WikiMarkdownRenderer
            wikiMarkdownRenderer;

    public PublicWikiController(
            ListPublishedWikiArticlesUseCase
                    listPublishedArticlesUseCase,
        ...
        this.listPublishedArticlesUseCase =
                listPublishedArticlesUseCase;

        this.getPublishedArticleUseCase =
                getPublishedArticleUseCase;

        this.articleTypePathMapper =
                articleTypePathMapper;

        this.wikiMarkdownRenderer =
                wikiMarkdownRenderer;
    }
    ...
    private ArticleType resolveOptionalArticleType(
            String articleTypePath
    ) {
        if (
                articleTypePath == null
                || articleTypePath.isBlank()
        ) {
            return null;
        }

        return articleTypePathMapper.fromPath(
                articleTypePath
        );
    }
}