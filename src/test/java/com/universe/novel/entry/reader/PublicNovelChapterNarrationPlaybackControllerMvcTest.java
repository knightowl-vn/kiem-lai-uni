...
class PublicNovelChapterNarrationPlaybackControllerMvcTest {
    ...
    @Test
    @WithMockUser
    @DisplayName("1. Supplementary route check: POST /segments/{segmentId}/prepare is not mapped on playback controller")
    void shouldReturn404ForLegacySegmentPrepareEndpoint() throws Exception {
        // Supplementary check: confirms PublicNovelChapterNarrationPlaybackController does not handle legacy segment prepare
    ...
    @Test
    @WithMockUser
    @DisplayName("2. Supplementary route check: GET /narration/manifest is not mapped on playback controller")
    void shouldReturn404ForLegacyManifestEndpoint() throws Exception {
        // Supplementary check: confirms PublicNovelChapterNarrationPlaybackController does not handle legacy manifest
        mockMvc.perform(get("/api/novel/chapters/" + CHAPTER_ID + "/narration/manifest"))
                .andExpect(status().isNotFound());
    }

    @Test
    @WithAnonymousUser
    @DisplayName("3. Legacy GET /narration/manifest is not permitted anonymously and redirects to login")
    void shouldRejectAnonymousAccessToRetiredManifestEndpoint() throws Exception {
        mockMvc.perform(get("/api/novel/chapters/" + CHAPTER_ID + "/narration/manifest"))
    ...
    @Test
    @WithAnonymousUser
    @DisplayName("4. Removed /playback alias is not a valid endpoint for anonymous access")
